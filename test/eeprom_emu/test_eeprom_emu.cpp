...
EepromFlashDriver driver;

void mem_dump(uint8_t* memory)
{
    for (uint32_t i = 0; i < driver.BankSize; i++)
    {
    ...
    for (uint32_t i = driver.BankSize; i < driver.BankSize*2; i++)
    {
        printf("0x%.2X ", memory[i]);
        if ((i & 0x0F) == 0x0F) printf("\n");
    }
...
void test_eeprom_write() {
    ...
    uint8_t expected[] = {
        // Bank header
        0xEE, 0x77, 0x99, 0x44, 0xFF, 0xFF, 0xFF, 0xFF,
...
void test_eeprom_bank_move() {
    ...
    uint16_t capacity = (eeprom.flash.BankSize - 16) / 8;
...
void test_crc16() {
    ...
    TEST_ASSERT_EQUAL_HEX16(0x29B1, eeprom.crc16(data, 9));
}
