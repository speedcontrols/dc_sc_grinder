...
__INLINE void fft_forward(fft_complex_t data[], unsigned bits) {
...
// Permutation must be performed prior to (DIT)/after (DIF) call
__INLINE void fft_inverse(fft_complex_t data[], unsigned bits) {
  unsigned size = 1 << bits;
#ifdef FFT_DIT
  unsigned shift = SINE_BITS + 1;
  for(unsigned stride = 2 ; stride <= size; stride <<= 1, shift--) {
...
__INLINE void fft_convert(fft_complex_t data[], unsigned bits, bool permutated, bool invert) {
  unsigned size = 1 << --bits;
...
__INLINE void fft_permutate(fft_complex_t data[], unsigned bits) {
  unsigned size  = 1 << bits;
...
// A data set built with this method does not require fft_permutate before DIT IFFT
// A data set built with this method does not require fft_convert before IFFT
__INLINE void fft_real_phase_magnitude(fft_complex_t complex[], unsigned bits, unsigned index, int32_t mag_lo, int32_t pha_lo, int32_t mag_hi, int32_t pha_hi) {
  unsigned size = 1 << bits;
...
// A data set built with this method does not require fft_permutate before DIT IFFT
// A data set built with this method does not require fft_convert before IFFT
__INLINE void fft_real_magnitude(fft_complex_t complex[], unsigned bits, unsigned index, int32_t mag_lo, int32_t mag_hi) {
  unsigned shift = SINE_BITS - bits + 1;
#ifdef FFT_DIT
  unsigned n = RBITS(index, bits);
