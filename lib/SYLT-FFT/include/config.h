...
#define SYLT_FFT_SINE_PRINTOUT  0 // Write sine table to screen (PC only)
...
// FFT is faster when SYLT_FFT_SINE_TABLE_CUSTOM set (located in RAM)

// LUT for sine wave, first quadrant only
#if SYLT_FFT_SINE_TABLE_CUSTOM

#ifndef SINE_BITS
#define SINE_BITS 7 // Sine quality (2..14) vs. memory tradeoff
#endif

#ifdef SYLT_FFT_SINE_TABLE__CONST
extern const int32_t sinetable[(1 << SINE_BITS) + 1];
else
// RAM
