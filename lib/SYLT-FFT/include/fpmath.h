...
__INLINE void sine_init() {
#if SYLT_FFT_SINE_TABLE_CUSTOM
  unsigned int n;
#if SYLT_FFT_SINE_PRINTOUT
  printf("// ROM\n");
...
__INLINE int32_t sine(uint32_t pos) {
...
// Fixed point pow(2, e)
__INLINE uint64_t fpow2(uint32_t e) {
...
__INLINE unsigned bin2gray(unsigned bits) {
  return (bits >> 1) ^ bits;
}

// Convert gray-code to binary
__INLINE unsigned gray2bin(unsigned bits) {
  bits ^= bits >> 16;
  bits ^= bits >>  8;
  bits ^= bits >>  4;
  bits ^= bits >>  2;
